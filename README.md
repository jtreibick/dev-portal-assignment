...
[Repository](https://github.com/jtreibick/dev-portal-assignment) · [Prototype](https://jtreibick.github.io/dev-portal-assignment/app.html) · [Backlog](https://github.com/jtreibick/dev-portal-assignment/issues?q=is%3Aissue%20is%3Aopen%20sort%3Acreated-asc)

## Executive summary

AI is accelerating the path from an idea to a working application, but the manual deployment process required to actually ship is becoming an increasingly visible bottleneck in the SDLC. Deploying and operating enterprise cloud infrastructure requires careful consideration and strict controls. When developers are ready to deploy into enterprise platform-managed infrastructure, their path to production is often not clear. They may need answers from documentation, service catalogs, infrastructure-as-code (IaC) modules, policies, CI/CD systems, and manual conversations and approvals with platform teams. In addition to being time-consuming, this coordination burden sits with the developer and adds drag to the rapidly increasing velocity of the SDLC process.

This proposal explores an agent-powered internal developer portal that guides a developer from a selected application repository to a reviewable infrastructure pull request (PR). The portal uses available application context from code repositories to infer a structured profile, asks questions needed to fill important gaps, recommends an approved infrastructure pattern, explains the relevant guardrails, validates the proposed change through existing deterministic systems, and routes it through approval workflows.

The AI helps a developer understand options and prepare a proposal. It does not become the authority for policy, approval, or execution. Existing internal developer portal, IaC, CI/CD, identity, and policy systems remain authoritative and are orchestrated rather than replaced. This is a focused hypothesis: make a common infrastructure request more self-service, reviewable, and trustworthy. The proposal does not claim to manage the full software delivery lifecycle.
...
- [Click-through prototype](https://jtreibick.github.io/dev-portal-assignment/app.html) — a self-contained, static workflow using sample data.
